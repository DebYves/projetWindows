...
namespace NamRider.ViewModel
{
    public class DrivingInfoPageViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        {
            //faut rafraichir la page, comme c'est un changement dans la BD et non dans l'objet, le binding mode twoway ne marche pas
            var service = new DrivingInfoService();
            var newThis = await service.GetDrivingById(SelectedDrivingInfo.Id);
            _navigationService.NavigateTo("DrivingInfoPage",newThis);
        }
        ...
        {
            if (Utils.StringDefined(ApiConstants.Token))
            {
