...
namespace NamRider.API.NamRiderDBAccess
{
    public class ParkingInfoDBMethod
    {
        ...
        /// <returns></returns>
        public AspNetUser GetAlertUser(int idParking, string userId)
        {
            var parking = (from m in context.ParkingInfoes where m.Id.Equals(idParking) select m).Single();
            return (from m in parking.AspNetUsers where m.Id.Equals(userId) select m).Single();
        }
